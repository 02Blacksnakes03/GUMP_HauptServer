...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			if (PlayerData.job and PlayerData.job.name == 'redlinep') or not Config.IsMechanicJobOnly then
				for k,v in pairs(Config.Zones) do
					if GetDistanceBetweenCoords(coords, v.Pos2.x, v.Pos2.y, v.Pos2.z, true) < v.Size.x and not lsMenuIsShowed then
						isInLSMarker  = true
			...
			if (PlayerData.job and PlayerData.job.name == 'redlinep') or not Config.IsMechanicJobOnly then
				for k,v in pairs(Config.Zones) do
					if GetDistanceBetweenCoords(coords, v.Pos3.x, v.Pos3.y, v.Pos3.z, true) < v.Size.x and not lsMenuIsShowed then
						isInLSMarker  = true
						ESX.ShowHelpNotification(v.Hint)
						break
					else
			...
			if (PlayerData.job and PlayerData.job.name == 'redlinep') or not Config.IsMechanicJobOnly then
				for k,v in pairs(Config.Zones) do
					if GetDistanceBetweenCoords(coords, v.Pos4.x, v.Pos4.y, v.Pos4.z, true) < v.Size.x and not lsMenuIsShowed then
						isInLSMarker  = true
						ESX.ShowHelpNotification(v.Hint)
						break
					else
			...
			if (PlayerData.job and PlayerData.job.name == 'redlinep') or not Config.IsMechanicJobOnly then
				for k,v in pairs(Config.Zones) do
					if GetDistanceBetweenCoords(coords, v.Pos5.x, v.Pos5.y, v.Pos5.z, true) < v.Size.x and not lsMenuIsShowed then
						isInLSMarker  = true
						ESX.ShowHelpNotification(v.Hint)
						break
					else
