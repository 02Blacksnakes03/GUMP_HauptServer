...
-- NORMAL
RegisterServerEvent('handy:server:setvalues')
AddEventHandler('handy:server:setvalues', function(source, background, darkbackground)
    backgroundurl = background
    darkbackgroundurl = darkbackground
end)

RegisterServerEvent('handy:server:getphonedata')
AddEventHandler('handy:server:getphonedata', function(source)
    ...
    MySQL.Async.fetchAll("SELECT * FROM `phone_information` WHERE `identifier` = @identifier", {["@identifier"] = xPlayer.identifier}, function(result)
        if result[1] ~= nil then
            MySQL.Async.fetchAll('SELECT * FROM jobs ', {}, function(jobs)
                Wait(100)
                TriggerClientEvent("handy:client:showphone", _source, result[1], jobs)
            end)
        else
            ...
            Wait(100)
            TriggerEvent("handy:server:getphonedata", _source)
        end
    end)
end)

RegisterServerEvent('handy:server:checkphone')
AddEventHandler('handy:server:checkphone', function(source)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local item = xPlayer.getInventoryItem("phone")

    if item.count >= 1 then
        TriggerClientEvent("handy:client:hasphone", _source)
    end
end)

RegisterServerEvent('handy:server:checkphonenumber')
AddEventHandler('handy:server:checkphonenumber', function(source)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    ...
    MySQL.Async.fetchAll("SELECT * FROM `phone_contacts` WHERE `identifier` = @identifier", {["@identifier"] = xPlayer.identifier}, function(result)
        if result[1] ~= nil then
            ...
            TriggerClientEvent("handy:client:hasnumber", _source, xPlayernumber)
        else
            ...
            MySQL.Async.fetchAll("SELECT * FROM `users` WHERE `phone_number` = @phone_number", {["@phone_number"] = number}, function(numberr)
                if numberr[1] == nil then
                    ...
                    TriggerClientEvent("handy:client:hasnumber", _source, number)
                else
                    TriggerEvent("handy:server:checkphonenumber", _source)
                end
            end)
        end
    end)
...
-- CONTACT
RegisterServerEvent('handy:server:addcontact')
AddEventHandler('handy:server:addcontact', function(source, name, number2)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    ...
    MySQL.Async.fetchAll("SELECT * FROM `users` WHERE `phone_number` = @phone_number", {["@phone_number"] = number}, function(result)
        if result[1] ~= nil then
            ...
            Wait(50)
            TriggerEvent("handy:server:loadcontact", _source)
        else
...
RegisterServerEvent('handy:server:addcontactfavourit')
AddEventHandler('handy:server:addcontactfavourit', function(source, name, number2)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local number = string.format(number2)
...
RegisterServerEvent('handy:server:editcontact')
AddEventHandler('handy:server:editcontact', function(source, name, number3, name2, number4)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('handy:server:deletecontact')
AddEventHandler('handy:server:deletecontact', function(source, name, number2)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local number = string.format(number2)
...
RegisterServerEvent('handy:server:sharecontact')
AddEventHandler('handy:server:sharecontact', function(source, name, number)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)

    TriggerClientEvent("handy:client:sharecontact", _source, _U("sharedcontact"), number)
end)

RegisterServerEvent('handy:server:loadcontact')
AddEventHandler('handy:server:loadcontact', function(source)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    ...
    MySQL.Async.fetchAll("SELECT * FROM `phone_contacts` WHERE `identifier` = @identifier", {["@identifier"] = xPlayer.identifier}, function(result)
        if (result) then
            ...
            TriggerClientEvent("handy:client:loadcontact", _source, html)
        else
            local html = ""
...
TriggerEvent('es:addGroupCommand', 'sca', 'user', function(p, args, user)
    TriggerClientEvent("handy:client:acceptsharecontact", p)
end)

TriggerEvent('es:addGroupCommand', 'scd', 'user', function(p, args, user)
    TriggerClientEvent("handy:client:declinesharecontact", p)
end)


-- MESSAGES
RegisterServerEvent('handy:server:loadmessage')
AddEventHandler('handy:server:loadmessage', function(source, sender)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local allmessages = {}
    local havecontacts
        ...
        MySQL.Async.fetchAll("SELECT * FROM `phone_contacts` WHERE `identifier` = @identifier", {["@identifier"] = xPlayer.identifier}, function(contact)
            ...
            MySQL.Async.fetchAll("SELECT * FROM `phone_messages` WHERE `receiver` = @receiver AND `sender` = @sender", {["@receiver"] =  result[1].phone_number, ["@sender"] = sender}, function(message)
                MySQL.Async.fetchAll("SELECT * FROM `phone_messages` WHERE `receiver` = @receiver AND `sender` = @sender", {["@receiver"] = sender, ["@sender"] =  result[1].phone_number}, function(sendersql)
                    ...
                    TriggerClientEvent("handy:client:loadmessages", _source, html)
                end)
            end)
        end)
    end)
...
RegisterServerEvent('handy:server:loadrecentmessage')
AddEventHandler('handy:server:loadrecentmessage', function(source)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local allmessages = {}
    local contacts = {}
    ...
    MySQL.Async.fetchAll("SELECT phone_number FROM `users` WHERE `identifier` = @identifier", {["@identifier"] = xPlayer.identifier}, function(result)
        MySQL.Async.fetchAll("SELECT * FROM `phone_contacts` WHERE `identifier` = @identifier", {["@identifier"] = xPlayer.identifier}, function(contact)
            ...
            MySQL.Async.fetchAll("SELECT * FROM `phone_messages` WHERE `receiver` = @receiver OR `sender` = @sender", {["@receiver"] = result[1].phone_number, ["@sender"] = result[1].phone_number}, function(message)
                ...
                TriggerClientEvent("handy:client:loadrecentmessages", _source, html)
            end)
        end)
    end)
end)

RegisterServerEvent('handy:server:sendmessage')
AddEventHandler('handy:server:sendmessage', function(source, message, number)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    ...
    MySQL.Async.fetchAll("SELECT phone_number FROM `users` WHERE `identifier` = @identifier", {["@identifier"] = xPlayer.identifier}, function(result)
        ...
        Wait(10)
        TriggerEvent("handy:server:loadmessage", _source, number)
    end)
    ...
    for i=1, #players do
        ...
        if zPlayer.job.label == number then
            TriggerClientEvent("handy:client:playbusinessmessagesound", zPlayer.source)
            isservicee = true
    ...
    if isservicee == false then
        getSourceFromIdentifier(Otheridentifier, function (Othersource)
            if Othersource ~= nil then
                TriggerClientEvent("handy:client:playmessagesound", Othersource)
...
RegisterServerEvent('handy:server:sendgps')
AddEventHandler('handy:server:sendgps', function(source, number, pos)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local time = os.date("%x") .. ' ' .. os.date("%H") .. ':' .. os.date("%M")
    MySQL.Async.fetchAll("SELECT phone_number FROM `users` WHERE `identifier` = @identifier", {["@identifier"] = xPlayer.identifier}, function(result)
...
-- CALL
RegisterServerEvent('handy:server:startcall')
AddEventHandler('handy:server:startcall', function(source, number)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local xPlayernumber = getNumberPhone(xPlayer.identifier)
    ...
    getSourceFromIdentifier(Otheridentifier, function (Othersource)
        if Othersource ~= nil then
            TriggerClientEvent("handy:client:callrequest", Othersource, _source, xPlayernumber, number, contactname)
        end
    end)

end)

RegisterServerEvent('handy:server:acceptcall')
AddEventHandler('handy:server:acceptcall', function(source, requeser, requesternumber, sourcenumber, callnumber)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    ...
    MySQL.Async.fetchAll("SELECT * FROM `phone_contacts` WHERE `identifier` = @identifier", {["@identifier"] = xPlayer.identifier}, function(contact)
        ...
        TriggerClientEvent("handy:client:acceptcall", source, requeser, requesternumber, callnumber, contactname)
    end)
end)

RegisterServerEvent('handy:server:loadrecentcalls')
AddEventHandler('handy:server:loadrecentcalls', function(source, number)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local xPlayernumber = getNumberPhone(xPlayer.identifier)
    ...
    Wait(250)
    TriggerClientEvent("handy:client:loadrecentcalls", _source, recentcalls)
end)


RegisterServerEvent('handy:server:acceptsalty')
AddEventHandler('handy:server:acceptsalty', function(source, requeser)
    exports['saltychat']:EstablishCall(requeser, source)
    exports['saltychat']:EstablishCall(source, requeser)
end)

RegisterServerEvent('handy:server:endsalty')
AddEventHandler('handy:server:endsalty', function(source, requeser)
    exports['saltychat']:EndCall(requeser, source)
    exports['saltychat']:EndCall(source, requeser)
end)

RegisterServerEvent('handy:server:endcall')
AddEventHandler('handy:server:endcall', function(source, requeser)
    TriggerClientEvent("handy:client:endcall", source)
    TriggerClientEvent("handy:client:endcall", requeser)
end)


RegisterServerEvent('handy:server:declinecall')
AddEventHandler('handy:server:declinecall', function(source, requeser)
    TriggerClientEvent("handy:client:declinecall", requeser)
end)

RegisterServerEvent('handy:server:stopcall')
AddEventHandler('handy:server:stopcall', function(source, number)
    local Otheridentifier = getIdentifierByPhoneNumber(number)

    getSourceFromIdentifier(Otheridentifier, function (Othersource)
        if Othersource ~= nil then
            TriggerClientEvent("handy:client:stopcall", Othersource)
        end
    end)
...
-- SETTINGS
RegisterServerEvent('handy:server:changewallpaper')
AddEventHandler('handy:server:changewallpaper', function(source, url)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    ...
    MySQL.Async.fetchAll("SELECT * FROM `phone_information` WHERE `identifier` = @identifier", {["@identifier"] = xPlayer.identifier}, function(result)
        if result[1] ~= nil then
            ...
            TriggerClientEvent("handy:client:changewallpaper", _source, url)
...
RegisterServerEvent('handy:server:changedarkmode')
AddEventHandler('handy:server:changedarkmode', function(source, state)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    ...
    MySQL.Async.fetchAll("SELECT * FROM `phone_information` WHERE `identifier` = @identifier", {["@identifier"] = xPlayer.identifier}, function(result)
        if result[1] ~= nil then
            ...
            TriggerClientEvent("handy:client:changedarkmode", _source, state)
            ...
            if result[1].wallpaper == Config.backgroundurl or result[1].wallpaper == Config.darkbackgroundurl or result[1].wallpaper == nil then
                if state == 1 then  
                    TriggerEvent("handy:server:changewallpaper", _source, Config.darkbackgroundurl )
                else
                    TriggerEvent("handy:server:changewallpaper", _source, Config.backgroundurl )
                end
            end
        else
...
TriggerEvent('es:addGroupCommand', 'testcall', 'admin', function(source, args, user)
    TriggerEvent("handy:server:startcall", source, args[1])
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Insufficient Permissions.' } })
end)

-- Business
RegisterServerEvent('handy:server:showBusiness')
AddEventHandler('handy:server:showBusiness', function(source, job)
    local players = ESX.GetPlayers()
    local _source = source
    ...
    Wait(250)
    TriggerClientEvent("handy:client:showBusiness", _source, member, onlinemember, motd)
end)

RegisterServerEvent('handy:server:loadbusinessrecentmessage')
AddEventHandler('handy:server:loadbusinessrecentmessage', function(source, job)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local allmessages = {}
    local contacts = {}
    MySQL.Async.fetchAll("SELECT phone_number FROM `users` WHERE `identifier` = @identifier", {["@identifier"] = xPlayer.identifier}, function(result)
            ...
            MySQL.Async.fetchAll("SELECT * FROM `phone_messages` WHERE `receiver` = @receiver OR `sender` = @sender", {["@receiver"] = job, ["@sender"] = job}, function(message)
                ...
                TriggerClientEvent("handy:client:loadbusinessrecentmessage", _source, html)
            end)
    end)
end)

RegisterServerEvent('handy:server:loadbusinessmessage')
AddEventHandler('handy:server:loadbusinessmessage', function(source, job, sender)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local allmessages = {}
    MySQL.Async.fetchAll("SELECT * FROM `phone_messages` WHERE `receiver` = @receiver AND `sender` = @sender", {["@receiver"] = job, ["@sender"] = sender}, function(message)
        MySQL.Async.fetchAll("SELECT * FROM `phone_messages` WHERE `receiver` = @receiver AND `sender` = @sender", {["@receiver"] = sender, ["@sender"] = job}, function(sendersql)
            ...
            TriggerClientEvent("handy:client:loadbusinessmessages", _source, html)
        end)
    end)
end)

RegisterServerEvent('handy:server:sendbusinessmessage')
AddEventHandler('handy:server:sendbusinessmessage', function(source, message, number, job)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local time = os.date("%x") .. ' ' .. os.date("%H") .. ':' .. os.date("%M")
        ...
        Wait(10)
        TriggerEvent("handy:server:loadbusinessmessage", _source, job, number)

        local Otheridentifier = getIdentifierByPhoneNumber(number)
        getSourceFromIdentifier(Otheridentifier, function (Othersource)
            if Othersource ~= nil then
                TriggerClientEvent("handy:client:playbusinessmessagesound", Othersource)
                TriggerClientEvent("d-notification", Othersource, _U("newservicemessage"))
            end
        end)
end)

RegisterServerEvent('handy:server:sendbusinessgps')
AddEventHandler('handy:server:sendbusinessgps', function(source, number, pos, job, position)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local time = os.date("%x") .. ' ' .. os.date("%H") .. ':' .. os.date("%M")
...
RegisterServerEvent('handy:server:changemotd')
AddEventHandler('handy:server:changemotd', function(source, job, motd)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('handy:server:sendservicemessage')
AddEventHandler('handy:server:sendservicemessage', function(source, rawmessage, job, sendnumber, gps, position, isDead)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local time = os.date("%x") .. ' ' .. os.date("%H") .. ':' .. os.date("%M")
    local xPlayernumber = getNumberPhone(xPlayer.identifier)
    ...
    if sendnumber == 1 then
       ...
       Wait(100)
        if isDead == nil then
            TriggerEvent("handy:server:loadmessage", _source, job)
        end
...
RegisterServerEvent('handy:server:loadservices')
AddEventHandler('handy:server:loadservices', function(source)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local html = ''
    ...
    Wait(250)
    TriggerClientEvent("handy:client:loadservices", _source, html)
end)


RegisterServerEvent('handy:server:serviceaccept')
AddEventHandler('handy:server:serviceaccept', function(source, job, number)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    ...
    local players = ESX.GetPlayers()
        for i=1, #players, 1 do
            local zPlayer = ESX.GetPlayerFromId(players[i])
            if zPlayer.job.label == job then
                TriggerClientEvent("handy:client:syncbpbabutton", zPlayer.source, number)
            end        
        end
end)

RegisterServerEvent('handy:server:serviceclose')
AddEventHandler('handy:server:serviceclose', function(source, job, number)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    ...
    local players = ESX.GetPlayers()
        for i=1, #players, 1 do
            local zPlayer = ESX.GetPlayerFromId(players[i])
            if zPlayer.job.label == job then
                TriggerClientEvent("handy:client:syncclosebmessage", zPlayer.source, number)
            end        
        end
end)
...
RegisterServerEvent('handy:server:twitter:open')
AddEventHandler('handy:server:twitter:open', function(source)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local twitteracount = GetUserTwitterAccount(xPlayer.identifier)

    if twitteracount == nil then
        TriggerClientEvent("handy:client:twitter:openregister", _source)
    else
        TriggerEvent("handy:server:twitter:load", _source)
        TriggerEvent("handy:server:twitter:loadlikes", _source)
    end
end)

-- Twitter Register
RegisterServerEvent('handy:server:twitter:registeraccount')
AddEventHandler('handy:server:twitter:registeraccount', function(source, username, userid, avatar)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    ...
    if hasid == nil then
        ...
        TriggerClientEvent("handy:client:twitter:accountsuccess", _source )
        TriggerEvent("handy:server:twitter:load", _source )
    else
...
RegisterServerEvent('handy:server:twitter:load')
AddEventHandler('handy:server:twitter:load', function(source)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local twitteracount = GetUserTwitterAccount(xPlayer.identifier)
    local tweets = {}
    ...
    TriggerClientEvent("handy:client:twitter:load", _source, html, twitteracount)
end)

RegisterServerEvent('handy:server:twitter:refresh')
AddEventHandler('handy:server:twitter:refresh', function(source)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local twitteracount = GetUserTwitterAccount(xPlayer.identifier)
    local tweets = {}
    ...
    TriggerClientEvent("handy:client:twitter:refresh", _source, html, twitteracount)
end)

-- Twitter Write Tweet
RegisterServerEvent('handy:server:twitter:writetweet')
AddEventHandler('handy:server:twitter:writetweet', function(source, message, twitteraccount, image)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local time = os.date("%x") .. ' ' .. os.date("%H") .. ':' .. os.date("%M")
...
RegisterServerEvent('handy:server:twitter:loadlikes')
AddEventHandler('handy:server:twitter:loadlikes', function(source, id)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local likedtweets = {}
    ...
    if likedtweets[1] ~= nil then
        TriggerClientEvent("handy:client:twitter:loadlikes", _source, likedtweets)
    end
end)

-- Twitter Tweets liketweet
RegisterServerEvent('handy:server:twitter:liketweet')
AddEventHandler('handy:server:twitter:liketweet', function(source, id)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local time = os.date("%x") .. ' ' .. os.date("%H") .. ':' .. os.date("%M")
    local players = ESX.GetPlayers()
    ...
    for i=1, #players do
        ...
        TriggerClientEvent("handy:client:twitter:updatelikes", zPlayer.source, id, tostring(newlikes))
    end
end)

-- Twitter Settings
-- Twitter Settings changepb
RegisterServerEvent('handy:server:twitter:changepb')
AddEventHandler('handy:server:twitter:changepb', function(source, avatar)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('handy:server:twitter:updateusername')
AddEventHandler('handy:server:twitter:updateusername', function(source, username)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
