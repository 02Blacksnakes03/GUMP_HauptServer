        ...
        if IsControlJustPressed(0,303) then
          ...
          if can_unlock then
            if unlock_procedure == "house" then
              ...
              if _s == false then
                TriggerEvent('glmp_singlenotify', 'green', 'Tür aufgeschlossen')
              elseif _s == true then
                TriggerEvent('glmp_singlenotify', 'red', 'Tür abgeschlossen')
              end
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), "door_creation_jobs", {
      ...
      function(data,menu)
        if data.current.value == "Done" then
          ...
          table.insert(elements,{label = "PD",value = "police"})
