		...
		elseif data.current.value == 'object_spawner' then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				elements = {
					...
					{label = _U('barrier'), model = 'prop_doghouse_01'},
					{label = _U('barrier'), model = 'prop_mp_barrier_01'},
			...
			if IsControlJustReleased(0, 38) then
				...
				if currentAction == 'shop_menu' then
					if PlayerData.job  then
						if PlayerData.job.name == 'police' or PlayerData.job.name == 'army' or PlayerData.job.name == 'fib' or PlayerData.job.name == 'lssd' then
							OpenJailer()
						end
					end
				end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			if IsControlJustPressed(0, 38) and PlayerData.job and PlayerData.job.name == 'police' then
		...
		if IsControlJustPressed(0,56) and PlayerData.job ~= nil and PlayerData.job.name == 'police' then
...
function SendPoliceDistressSignal()
	...
	TriggerServerEvent('esx_phone:send', 'police', "Ein Officer hat den Panicbutton gedrückt", false, {
		x = coords.x,
		y = coords.y,
		z = coords.z
	})
