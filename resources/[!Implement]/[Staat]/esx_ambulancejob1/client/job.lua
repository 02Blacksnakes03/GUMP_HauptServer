...
function OpenMobileAmbulanceActionsMenu1()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_ambulance_actions', {
					...
					if data.current.value == 'revive' then
						...
						ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if closestPlayerHealth <= 105 then
		...
		elseif data.current.value == 'dienstausweis_aktion' then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'dienstausweis_aktion', {
				...
				TriggerServerEvent('byte_dienstausweis:show', GetPlayerServerId(PlayerId()), GetPlayerServerId(player))
...
function SetVehicleMaxMods(vehicle)
	...
	local props = {
	  ...
	  modFrontBumper  = 2,
	  modTurbo        = true,
	  SetVehicleNumberPlateText(vehicle, "LSMD"),
	}
...
function OpenVehicleSpawnerMenu(type, station, part, partNum)
	...
	local elements = {
		...
		{label = 'Krankenwagen', action = 'ambulance'}
		

	}

	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle', {
		title    = 'Fraktionsgarage',
		align    = 'top-left',
		elements = elements
	}, function(data, menu)
