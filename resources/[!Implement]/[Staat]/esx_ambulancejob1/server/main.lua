...
function medicLog(message)
	local embed = {
		  {
			  ["color"] = '15158332',
			  ["title"] = "**Ambulance Action**",
			  ["description"] = message,
			  ["footer"] = {
				  ["text"] = 'German Life Multiplayer | GLMP.de',
			  },
		  }
	  }  
	PerformHttpRequest('https://discordapp.com/api/webhooks/803990278540623914/pGIEO3He148qvlysbmngvVGk9jiGl2T8fjD3Do80vQYHUcVuhBHEwkYr4_3ga8Kr8gh4', function(err, text, headers) end, 'POST', json.encode({username = 'GLMP - AmbulanceLogs', embeds = embed}), { ['Content-Type'] = 'application/json' })
end

RegisterServerEvent('esx_ambulancejob:revive')
AddEventHandler('esx_ambulancejob:revive', function(target)
	local xPlayer = ESX.GetPlayerFromId(source)
	local medic = GetPlayerName(source)
	local patient = GetPlayerName(target)
	if xPlayer.job.name == 'ambulance' then
		--xPlayer.addMoney(Config.ReviveReward)
		TriggerClientEvent('ds:hide', target, 1) 
		TriggerClientEvent('ds:hide', target, 2)
		TriggerClientEvent('esx_ambulancejob:revive', target)
		medicLog('Der Spieler **'.. medic ..' ('.. source ..')** hat den Spieler **'.. patient ..' ('.. target ..')** wiederbelebt.')
	else
...
function sendRevive(color, name, title, message, footer)
	local embed = {
		  {
			  ["color"] = color,
			  ["title"] = "**".. title .."**",
	...
	PerformHttpRequest('https://discordapp.com/api/webhooks/759212556295995452/2fYCLwcLh2k9VF8DvC2nhbNzs7rgVtBzxCvRT7gJPa8TqsREjvzCh9yIOgjMdVb5XPlN', function(err, text, headers) end, 'POST', json.encode({username = name, embeds = embed}), { ['Content-Type'] = 'application/json' })
end
