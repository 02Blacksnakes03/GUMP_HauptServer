...
AddEventHandler('esx_holdupbank:robberyStarted', function(currentStore)
	...
	if Stores[currentStore] then
		...
		if not rob then
			if cops >= Config.PoliceNumberRequired and fib >= Config.FIBNumberRequired then
				...
				for i=1, #xPlayers, 1 do
					...
					if xPlayer.job.name == 'police' or xPlayer.job.name == 'fib' then
						...
						TriggerClientEvent('esx_holdupbank:setBlip', xPlayers[i], Stores[currentStore].position)
					end
				end
				...
				TriggerClientEvent('esx_holdupbank:currentlyRobbing', _source, currentStore)
				...
				SetTimeout(store.secondsRemaining * 1000, function()
					if robbers[_source] then
						rob = false
						if xPlayer then
							TriggerClientEvent('esx_holdupbank:robberyComplete', _source, store.reward)
				...
				TriggerClientEvent('esx:showNotification', _source, _U('min_police'))
				Wait(3000)
				TriggerClientEvent('esx:showNotification', _source, _U('bohrer'))
			end
		else
