...
--[[shop = {
    {
        pos = { x = 1323.548, y =  -1652.308, z =  52.26306},
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        for k,v in pairs(shop) do
            ...
            local dist = Vdist(playerCoords.x, playerCoords.y, playerCoords.z, v.pos.x, v.pos.y, v.pos.z)
            ...
            if dist < 2.5 then
                ...
                if IsControlJustPressed(0, 38) then
                    ...
                    ----print("tattos handler loaded")

                    ESX.TriggerServerCallback("getTattos", function(tl) 
                        
                        ----print("tattos list loaded")
...
RegisterNUICallback("trigger", function(data)
    ...
    if data.event == "componentTriggerServerEvent" then
       if data.args[1] == "TattooShop" then 
            if data.args[2] == "requestTattooShopCategoryTattoos" then 

                ESX.TriggerServerCallback("requestTattoos", function(cb) 

                    SendNUIMessage({
                        action = "responseTattooShopCategory",
                        data = json.encode(cb)
...
AddEventHandler('skinchanger:modelLoaded', function()
	ESX.TriggerServerCallback('tattoo:requestPlayerTattoos', function(tattooList)
		if tattooList then
		...
		if data.current.value then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'tattoo_shop_categories', {
				...
				if data2.current.value ~= nil then
					...
					ESX.TriggerServerCallback('tattoo:purchaseTattoo', function(success)
						if success then
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not HasAlreadyEnteredMarker then
			...
			TriggerEvent('tattoo:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('tattoo:hasExitedMarker', LastZone)
		end
	end
end)

AddEventHandler('tattoo:hasEnteredMarker', function(zone)
...
AddEventHandler('tattoo:hasExitedMarker', function(zone)
	CurrentAction = nil
